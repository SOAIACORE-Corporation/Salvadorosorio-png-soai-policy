...
if ($stateBlobExists -ne 'true') {
    throw "Existing A2 state '$StateKey' was not found. Private migration plan requires completed provisioning."
}
...
Write-Boundary 'A2_RESOURCE_GROUP_EXISTS' 'PASS'

$timestamp = (Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ')
$artifactRoot = Join-Path $HOME ("SOAIACORE\controlled-a2-private-migration-plan\$timestamp")
...
$planPath = Join-Path $evidenceDir 'soa-intelligence-a2-private-migration.tfplan'
$receiptPath = Join-Path $artifactRoot 'SOA_INTELLIGENCE_A2_PRIVATE_MIGRATION_PLAN_RECEIPT.sanitized.json'
...
try {
    ...
    $stateAddresses = @(& terraform $chdirArg state list | Sort-Object)
    Assert-LastExitCode 'terraform state list'
    $stateDiff = @(Compare-Object -ReferenceObject $expectedPreState -DifferenceObject $stateAddresses)
    if ($stateDiff.Count -ne 0) {
        throw "A2 pre-state differs from the closed provisioning receipt: $($stateDiff | Out-String)"
    }
    ...
    if ($planExit -eq 0) {
        throw 'Terraform returned no changes; private migration resources are expected.'
    }
    if ($planExit -ne 2) {
        throw "terraform plan failed with exit code $planExit."
    }

    # Parse in memory only. Do not persist JSON because secret values can appear
    # in plan JSON even when marked sensitive.
    ...
    $exactActionSet = (
        $createAddresses.Count -eq 4 -and
    ...
    $receipt = [ordered]@{
        receipt_type = 'SOA_INTELLIGENCE_A2_PRIVATE_MIGRATION_GOVERNED_PLAN'
        generated_at_utc = (Get-Date).ToUniversalTime().ToString('o')
        config_commit = $localHead
        branch = $ExpectedBranch
        state_key = $StateKey
        pre_state_address_count = $stateAddresses.Count
        migration_worker_image = $MigrationWorkerImage
    ...
    Write-Host ''
    Write-Host '=== SOA INTELLIGENCE A2 · PRIVATE MIGRATION GOVERNED PLAN RECEIPT ==='
    Write-Boundary 'CONFIG_COMMIT' $localHead
    Write-Boundary 'STATE_KEY' $StateKey
    Write-Boundary 'PRE_STATE_ADDRESS_COUNT' ([string]$stateAddresses.Count)
    Write-Boundary 'MIGRATION_WORKER_IMAGE' $MigrationWorkerImage
    ...
    if (-not $exactActionSet) {
        throw 'A2 private-migration plan does not match exact 4-create action set. Apply is prohibited.'
    }

    Write-Boundary 'A2_PRIVATE_MIGRATION_PLAN_GATE' 'PASS'
